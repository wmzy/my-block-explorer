...
```json
{
  ...
  "scripts": {
    ...
    "dev:server": "tsx watch --env-file=.env src/server/app.ts",
    "build": "npm run build:server && npm run build:client",
    "build:client": "vite build --config vite.client.config.ts && npm run setup:spa",
    "build:server": "tsc --project tsconfig.server.json",
    "setup:spa": "echo '/* /index.html 200' > dist/client/_redirects",
    "start:server": "node dist/server/app.js",
    "preview:client": "vite preview --config vite.client.config.ts",
    "test": "vitest",
    "test:ui": "vitest --ui",
